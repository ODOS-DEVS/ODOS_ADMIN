...
type BackendPlatformLedgerEntry = {
  id: string;
  kind: string;
  direction: string;
  title: string;
...
function mapPlatformLedgerEntry(
  entry: BackendPlatformLedgerEntry,
): AdminPlatformLedgerEntry {
  return {
    id: entry.id,
    kind: entry.kind,
...
export async function getPaymentTransactionsPage(
  ...
  const raw = await requestJson<AdminPage<BackendPaymentTransaction> | BackendPaymentTransaction[]>(
    `/admin/finance/payments${buildAdminListQuery({ limit: ADMIN_PAGE_SIZE, ...params })}`,
    { token },
  );
  const page = normalizeAdminPageResponse(raw);
  return { ...page, items: page.items.map(mapPaymentTransaction) };
}

export async function getPaymentTransactions(token: string) {
  return fetchAllAdminPages((offset, limit) =>
    getPaymentTransactionsPage(token, { offset, limit }),
  );
}

export async function getPlatformLedgerEntriesPage(
  token: string,
  params: AdminListParams = {},
) {
  const raw = await requestJson<AdminPage<BackendPlatformLedgerEntry> | BackendPlatformLedgerEntry[]>(
    `/admin/finance/ledger${buildAdminListQuery({ limit: ADMIN_PAGE_SIZE, ...params })}`,
    { token },
  );
  const page = normalizeAdminPageResponse(raw);
  return { ...page, items: page.items.map(mapPlatformLedgerEntry) };
}

export async function getPlatformLedgerEntries(token: string) {
  return fetchAllAdminPages((offset, limit) =>
    getPlatformLedgerEntriesPage(token, { offset, limit }),
  );
}
