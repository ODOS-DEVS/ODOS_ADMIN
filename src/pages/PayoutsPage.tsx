...
export function PayoutsPage() {
  ...
  const [requests, setRequests] = useState<AdminVendorWithdrawalRequest[]>([]);
  ...
  const loadRequests = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await getVendorWithdrawalRequests(token);
      setRequests(result);
    } catch (loadError) {
      setError(
        loadError instanceof Error
          ? loadError.message
          : "Unable to load vendor payouts.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  if (isLoading) {
    return <LoadingState label="Loading vendor payouts..." />;
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Payouts"
        title="Vendor wallets and withdrawals"
        description="Review vendor withdrawal requests, approve them, and then trigger live payout transfers with a full audit trail."
      />
      ...
      >
        {filteredRequests.length === 0 ? (
          <EmptyState
            ...
            description="Vendor withdrawal requests will appear here as soon as vendors start moving money out of their ODOS wallet."
          />
        ) : (
